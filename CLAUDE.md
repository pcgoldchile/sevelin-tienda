...
propio), `SYNC_SECRET` (compartido con el POS), `FLOW_API_KEY`/`FLOW_SECRET_KEY` (sandbox
...
- Catálogo público + carrito (`localStorage`, checkout de invitado, **sin cuentas de cliente** —
  decisión consciente, ver "Pendiente" en SNAPSHOT si eso cambia).
- Checkout + pago con Flow + envío (Retiro en tienda / Despacho local en Arica / Chilexpress fuera
  de Arica, con tarifa mock mientras no haya API key real).
